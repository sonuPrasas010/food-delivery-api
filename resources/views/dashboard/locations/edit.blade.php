        ...
        <div class="col-12">
            <div class="card card-default">
                <div class="card-header card-header-border-bottom">
                    <h2>Edit {{ $deliveryLocation->name }}</h2>
                </div>
                <div class="card-body">
                    @if ($errors->any())
                    ...
                    <div class="ec-vendor-upload-detail">
                        <form class="row g-3" method="POST" action="{{ route('delivery-location.update',$deliveryLocation) }}">
                            @csrf
                            @method('put')
                            <div class="col-md-12">
                                <label for="name" class="form-label">Location Name</label>
                                <input name="name" value="{{ old('name') ?? $deliveryLocation->name }}"
                            ...
                            <div class="col-md-12 mt-3">
                                ...
                                <input type="number" id="cost" name="cost" value="{{ old('cost') ?? $deliveryLocation->shipping_cost }}" class="form-control {{ $errors->has('cost') ? 'is-invalid' : '' }}">
                                @error('cost')
