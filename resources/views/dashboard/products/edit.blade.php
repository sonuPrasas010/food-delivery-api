...
@section('content')
    ...
    <form action="{{ route('product.update',$food) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('put')
        @if ($errors->any())
            ...
            <div class="col-12">
                <div class="card card-default">
                    <div class="card-header card-header-border-bottom">
                        <h2>Edit {{ "$food->food_name"." (".$food->vendor->vendor_name.")" }}</h2>
                    </div>
                    <div class="card-body">
                                        ...
                                        <div class="avatar-upload col-12">
                                            ...
                                            <div class="avatar-preview ec-preview">
                                                <div class="imagePreview ec-div-preview">
                                                    <img class="ec-image-preview" src="{{ $food->image_url  }}"
                                                        alt="edit">
                                                </div>
                                            </div>
                                        </div>
                                            ...
                                            <div class="thumb-upload">
                                                ...
                                                <div class="thumb-preview ec-preview">
                                                    <div class="image-thumb-preview">
                                                        <img class="image-thumb-preview ec-image-preview" src="{{ asset($food->small_image_url) }}"alt="edit">
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                            ...
                            <div class="col-lg-8">
                                <div class="ec-vendor-upload-detail row">
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <input class="form-control {{ $errors->has('name')? 'is-invalid' :'' }}" id="product-name" name="name" value="{{ old('name') ?? $food->food_name }}">
                                        @error('name')
                                    ...
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <input id="slug" name="slug" class="form-control here set-slug {{ $errors->has('slug')? 'is-invalid' : '' }}" value="{{ old('slug') ?? $food->slug }}">
                                        @error("slug")
                                        <span class="invalid-feedback">{{ $message }}</span>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="col-md-12 mb-3">
                                        ...
                                        <textarea class="form-control {{ $errors->has('short_description') ? 'is-invalid' : '' }}" rows="2" id="short_description"
                                            name="short_description">{{ old('short_description')?? $food->short_desc }}</textarea>
                                    ...
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <input name="price" type="number" class="form-control {{ $errors->has('price') ? 'is-invalid' : '' }}" id="price1" value="{{ old('price') ?? $food->price }}">
                                        @error('price')
                                           <span class="invalid-feedback"> {{$message}}</span>
                                        @enderror
                                    </div>
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <input name="discount" type="number" step=".1" class="form-control {{ $errors->has('discount') ? 'is-invalid':'' }}" id="discount1" value="{{ old('discount') ?? $food->discount }}">
                                        @error("discount")
                                        <span class="invalid-feedback">{{ $message }}</span>
                                        @enderror
                                    </div>
                                    <div class="col-md-12 mb-3">
                                        ...
                                        <textarea name="long_description" id="description" class="form-control {{ $errors->has('long_description') ? 'is-invalid' : '' }}" rows="4"> {{ old('long_description')?? $food->long_desc }}</textarea>
                                            ...
                                            <optgroup label="Food">
                                                @foreach ($vendors as $vendor)
                                                    @if ($vendor->category_id == 1)
                                                        <option value="{{ $vendor->vendor_id }}" {{ $food->vendor_id == $vendor->vendor_id ? 'selected' : '' }}>
